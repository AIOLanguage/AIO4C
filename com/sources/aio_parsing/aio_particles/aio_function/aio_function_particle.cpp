...
void aio_function_particle::monitor_function_modifier(const char symbol, const unsigned position)
{
...
#endif
            //Create field:
            this->function_ptr = new aio_function();
...
void aio_function_particle::monitor_function_name(const char symbol, const unsigned position)
{
    //기호를 확인하다:
    const bool is_whitespace_cond = is_space_or_line_break(symbol);
...
#endif
            //Create field:
            this->function_ptr->name = new str_hook(this->token_holder);
...
void aio_function_particle::monitor_function_args(const char symbol, const unsigned position)
{
    //Find arg scope:
    const str_hook *arg_hook = explore_hook_scope(position, OPENING_PARENTHESIS, CLOSING_PARENTHESIS, this->radius_ptr);
#ifdef AIO_FUNCTION_PARTICLE_DEBUG
    ...
    //Set arg count:
    this->function_ptr->arg_count = args_count;
    //------------------------------------------------------------------------------------------------------------------
    //찌꺼기 수집기:
...
void aio_function_particle::monitor_function_output_type(const char symbol, const unsigned position)
{
    //기호를 확인하다:
    const bool is_whitespace_cond = is_space_or_line_break(symbol);
...
void aio_function_particle::monitor_function_body(const char symbol, const unsigned position)
{
    const str_hook *body_hook = explore_hook_scope(position, OPENING_BRACE, CLOSING_BRACE, this->radius_ptr);
#ifdef AIO_FUNCTION_PARTICLE_DEBUG
...
//    this->function = function_orbit.spin(body_hook);
