...
aio_dou_result *make_plus_or_minus(aio_file *object, aio_method_definition *method_definition,
                                   aio_method_container *method_container, char *code_line);

aio_dou_result *make_multiplication_or_division(aio_file *object, aio_method_definition *method_definition,
                                                aio_method_container *method_container, char *code_line);

aio_dou_result *make_brackets(aio_file *object, aio_method_definition *method_definition,
                              aio_method_container *method_container, char *code_line);

aio_dou_result *make_method_or_variable(aio_file *object, aio_method_definition *method_definition,
                                        aio_method_container *method_container, char *code_line);

aio_dou_result *make_method(char *method_name, aio_dou_result *dou_result, aio_file *object);
...
aio_str parse_dou_line_expression(aio_file *object, aio_method_definition *method_definition,
    ...
    aio_str str_result = double_to_str(dou_result->acc);
    return str_result;
}
    ...
    if (is_not_empty_string(method_or_variable_name) == 0) { // 무언가가 발견도면
        ...
        if (i < code_line_length && code_line[i] == '(' || code_line[i] == '~') {
            if (code_line[i] == '(') {
                ...
                char *acc_str_result = double_to_str(instead_method_result->acc);
...
aio_dou_result *
make_method(char *method_name, aio_dou_result *dou_result, aio_file *object) {
    //덩으롼 객체에있는 메소드로 작업하기:
