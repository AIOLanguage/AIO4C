...
void weave_loop_instruction_for(void *parent, const_string source_code,
    ...
    if (is_ready_for_weaving) {
        ...
        //Create init holder:
        aio_instruction_holder *init_holder = new_aio_local_instruction_holder(parent);
        aio_instruction_list *init_instruction_list = init_holder->instruction_list;
        aio_variable_definition_list *init_variable_definition_map = init_holder->variable_definition_map;
        //Create cycle holder:
        ...
        if (header_material_type == AIO_LOOP_MATERIALS_DEFAULT_HEADER) {
            ...
            aio_variable_definition *definition = get_variable_definition_in_function_tree(pointer_name, parent);
            if (definition == NULL) {
                ...
                add_aio_variable_definition_in_list(init_variable_definition_map, definition);
        ...
        aio_instruction *loop_instruction = new_aio_loop_instruction(parent, loop_condition, init_holder, cycle_holder);
        aio_instruction_list *parent_instruction_list = parent->instruction_list;
