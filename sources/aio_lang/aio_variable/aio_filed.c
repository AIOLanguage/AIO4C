...
aio_variable *new_aio_field_by_definition(struct aio_variable_definition *field_definition, struct aio_value *value)
{
    aio_variable *variable = new_object(sizeof(aio_variable));
    variable->definition = field_definition;
    variable->value = value;
    variable->init_type = AIO_VARIABLE_NOT_INITIALIZED;
    return variable;
}

void free_aio_field(struct aio_filed *filed)
{
    free_aio_value(filed->value);
    free(filed);
}
