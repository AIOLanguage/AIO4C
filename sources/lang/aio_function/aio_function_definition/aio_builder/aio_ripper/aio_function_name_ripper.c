...
string dig_function_name(const_string source_code, int *pointer_reference) {
    ...
    for (int i = *pointer_reference; i < source_code_length; ++i) {
        ...
        const_boolean is_space_or_break_line_or_open_parenthesis = is_space(symbol) || is_line_break(symbol)
        ...
        //독서 중지 (Stop reading):
        if (is_space_or_break_line_or_open_parenthesis && watcher->mode == POINT_READING_MODE) {
            watcher->end_index = i;
            *pointer_reference = i;
            break;
        }
