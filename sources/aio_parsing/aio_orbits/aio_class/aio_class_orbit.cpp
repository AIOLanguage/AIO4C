...
static array_list<aio_particle<aio_class>> *new_class_particle_list(aio_runtime *runtime, aio_class *clazz)
{
    array_list<aio_particle<aio_class>> *particle_list = new array_list<aio_particle<aio_class>>();
    particle_list->add(new aio_class_particle<aio_class>(runtime));
    particle_list->add(new aio_field_particle<aio_class>(runtime->get_types(), clazz->field_definition_list));
    particle_list->add(new aio_scope_particle<aio_class>());
    particle_list->add(new aio_function_particle<aio_class>());
    particle_list->add(new aio_import_particle<aio_class>(runtime->get_file_list()));
